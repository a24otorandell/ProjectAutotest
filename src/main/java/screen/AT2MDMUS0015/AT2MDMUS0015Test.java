...
public class AT2MDMUS0015Test {
    ...
    protected boolean testCSED(TestDriver driver) {
        ...
        if (!searchHard(driver, "user2")) return false;
        if (!delete(driver)) return false;
        if (!searchHard(driver, "user")) return false;
        if (!delete(driver)) return false;
        return true;
    }
    ...
    public boolean searchHard (TestDriver driver, String user) {
        if (!Functions.simpleClick(driver,
