...
public class AT2ACCSU0006Test {
    ...
    private boolean Multi_lenguaje_search (TestDriver driver){
        ...
        if(!Functions.getText(driver,new String[]{"multi_lenguaje_se_e_lenguage",getElements("multi_lenguaje_se_e_lenguage")}, // element path
                "lenguaje_code_desc", // key for data value (the name)
                " on ADD")){
            return false;
        }
        ...
        if(!Functions.insertInput(driver,new String[]{"multi_lenguaje_se_i_lenguage",getElements("multi_lenguaje_se_i_lenguage")},
                "lenguaje_code_desc",data.getData().get("lenguaje_code_desc")," on QBE")){
            return false;
        }
    ...
    private boolean Rates_maintenance_add (TestDriver driver){
        ...
        if(!Functions.checkExistence(driver,
                "", //value
                "code",//data name (x)
                new String[]{"rates_maintenance_se_i_code",getElements("rates_maintenance_se_i_code")},//input
                new String[]{"rates_maintenance_se_b_search",getElements("rates_maintenance_se_b_search")},//search button
                new String[]{"rates_maintenance_se_e_result",getElements("rates_maintenance_se_e_result")},//expected not found result
                "both",//type of input
                3, //value length
                " on ADD")){
            return false;
