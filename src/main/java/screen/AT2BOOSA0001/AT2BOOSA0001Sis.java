...
public class AT2BOOSA0001Sis {
    ...
    protected boolean testCSED (TestDriver driver){
        ...
        if(!Search_booking_simple_search(driver)){return false;}
        if(!Getrecords(driver)){return false;}
        if(!Hard_qbe(driver)){return false;}
        return false;
    }

    //SEARCH BOOKING
    private boolean Hard_qbe (TestDriver driver){
        String where=" on HARD QBE";
        driver.getReport().addHeader("HARD QBE SEARCH IN SEARCH BOOKING",3,false);
        ...
        if (!Functions.insertInput(driver, new String[]{"query_i_booking_reference",getElements("query_i_booking_reference")},
                "result_receptive", getData("result_receptive"), where)){return false;}
  ...
  /*      Functions.insertInput(driver,
        ...
        if (!Functions.enterQueryAndClickResult(driver,
                new String[]{"query_i_booking_reference", getElements("query_i_booking_reference")}, //any query input
                new String[]{"query_i_receptive", getElements("query_i_receptive")}, //table result
    ...
    private boolean Getrecords (TestDriver driver){
        ...
        if (!Functions.zoomOut(driver, 4)){return false;}
    ...
    private boolean Search_booking_simple_search (TestDriver driver){
        ...
        if (!Functions.insertInput(driver, new String[]{"query_simple_i_creation_date",getElements("query_simple_i_creation_date")},
                "create_date",  DataGenerator.getRelativeDateToday("dd/MM/yyyy", 0, DataGenerator.random(0,-3), 0), where)){return false;}
        Functions.break_time(driver,25,500);
